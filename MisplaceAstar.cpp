...
using namespace std;
//map <key,value> x;
//map to keep the track of visited states
...
//map to keep the track of parent state of each state
		...
		for (int j=0; j<3; j++)
		{
			if(a[i][j] != 0)
			{
				for (int k=0; k<3; k++)
				{
					for (int l=0; l<3; l++)
					{
						if(a[i][j]==goal[k][l] && i!=k && j!=l)
							dist+=1; //absolute value since it may be negative
					}
				}
			}
		}
...
void solve(vector<vector<int> > a, int moves)
{
	...
	Q.push(state(a,moves));//type casting
	while(!Q.empty())
	{
		...
		vector<vector<vector<int> > > ns = neighbours(s);//more than one state in it 
		vector<vector<vector<int> > >::iterator it;
		for(it=ns.begin();it!=ns.end();it++)
		{
			vector<vector<int> > temp = *it;//value of pointer storing in another vector of 2D type
			if(!visit(temp))
			{
